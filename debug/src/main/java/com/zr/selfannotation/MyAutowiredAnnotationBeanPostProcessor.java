...
public class MyAutowiredAnnotationBeanPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
	...
	public MyAutowiredAnnotationBeanPostProcessor() {
		...
		try {
			this.autowiredAnnotationTypes.add((Class<? extends Annotation>)
					ClassUtils.forName("javax.inject.Inject", MyAutowiredAnnotationBeanPostProcessor.class.getClassLoader()));
	...
	public void setBeanFactory(BeanFactory beanFactory) {
		if (!(beanFactory instanceof ConfigurableListableBeanFactory)) {
			throw new IllegalArgumentException(
					"MyAutowiredAnnotationBeanPostProcessor requires a ConfigurableListableBeanFactory: " + beanFactory);
		}
	 ...
	 * 	如果只有一个Autowired required=false 没有默认构造，会报异常
	 * 	如果没有Autowired注解，定义了两个及其以上的有参构造方法，没有无参构造方法，就报错
					...
					if (value != null || this.required) {
						...
						if (autowiredBeanNames.size() == 1) {
							...
							if (beanFactory.containsBean(autowiredBeanName) &&
								...
								cachedFieldValue = new MyAutowiredAnnotationBeanPostProcessor.ShortcutDependencyDescriptor(
