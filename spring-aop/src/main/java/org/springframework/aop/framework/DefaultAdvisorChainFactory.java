...
public class DefaultAdvisorChainFactory implements AdvisorChainFactory, Serializable {
	...
	public List<Object> getInterceptorsAndDynamicInterceptionAdvice(
		...
		// AdvisorAdapterRegistry这个类的主要作用是将Advice适配Advisor 将Advisor适配为对应的MethodInterceptor
		...
		for (Advisor advisor : advisors) {
			...
			if (advisor instanceof PointcutAdvisor) {
				...
				// 如果提前进行过切点的匹配或者当前的Advisor适用于目标类
				if (config.isPreFiltered() || pointcutAdvisor.getPointcut().getClassFilter().matches(actualClass)) {
					...
					if (match) {
						// 拦截器链是通过AdvisorAdapterRegistry来加入的，这个AdvisorAdvisorAdapterRegistry对advice织入具有很大作用
						MethodInterceptor[] interceptors = registry.getInterceptors(advisor);
						// 使用MethodMatchers的matches方法进行匹配判断
						if (mm.isRuntime()) {
							// Creating a new object instance in the getInterceptors() method
							// isn't a problem as we normally cache created chains.
							// 动态切点则会创建一个InterceptorAndDynamicMethodMatcher对象
