...
final class PostProcessorRegistrationDelegate {
	...
	/**
	 * 方法名字叫invokeBeanFactoryPostProcessors
	 * 但实际上调用了BeanFactoryPostProcessor和BeanDefinitionRegistryPostProcessor
	 * 并且先调用顺序如下
	 * （spring）外部实现了bdrpp的子类 -> spring内部实现了bdrpp的子类，然后调用bdrpp ->
	 * priorityOrdered的bfpp  -> Orderd的bfpp -> 没有实现任何排序接口的对象 中的postProcessorBeanFactory
	 */
	public static void invokeBeanFactoryPostProcessors(
		...
		// 将已经执行过的BeanFactoryPostProcessor存储在processedBeans中，防止重复执行
		Set<String> processedBeans = new HashSet<>();

		// 判断beanFactory是否是BeanDefinitionRegistry类型，此处是DefaultListableBeanFactory，
		// 实现了BeanDefinitionRegistry接口，所以为true
		if (beanFactory instanceof BeanDefinitionRegistry) {
			// 类型转换
			BeanDefinitionRegistry registry = (BeanDefinitionRegistry) beanFactory;
			// 这两个接口是不同的，BeanDefinitionRegistryPostProcessor是BeanFactoryPostProcessor的子集
			// BeanFactoryPostProcessor主要针对的操作对象是BeanFactory，
			// 而BeanDefinitionRegistryPostProcessor主要针对的对象是BeanDefinition
			// 存放BeanFactoryProcessor的集合
			...
			// 找到所有实现BeanDefinitionRegistryPostProcessor接口bean的beanName，此处需要重复查找的原因，在于上面的执行过程中可能会新增其他的BeanDefinitionRegistryPostProcessor
			...
			// 调用入参BeanFactoryPostProcessor中的普通BeanFactoryPostProcessor的postProcessBeanFactory方法
		...
		// 下面开始处理容器中所有BeanFactoryPostProcessor
		// 可能会包含一些实现类，只实现了BeanFactoryPostProcessor，并没有实现BeanDefinitionRegistryPostProcessor
		...
		// 存放，实现了Ordered接口的BeanFactoryPostProcessor的beanName
		List<String> orderedPostProcessorNames = new ArrayList<>();
		// 存放，普通BeanFactoryPostProcessor的beanName
		List<String> nonOrderedPostProcessorNames = new ArrayList<>();
		// 遍历postProcessorNames,将BeanFactoryPostProcessor按实现PriorityOrdered、实现Ordered、普通三种区分开
		for (String ppName : postProcessorNames) {
		...
		// 遍历实现了PriorityOrdered接口的BeanFactoryPostProcessor,执行ostProcessBeanFactory方法
		...
		// 遍历实现了Ordered接口的BeanFactoryPostProcessor,执行ostProcessBeanFactory方法
		invokeBeanFactoryPostProcessors(orderedPostProcessors, beanFactory);

		// Finally, invoke all other BeanFactoryPostProcessors.
		// 创建存放普通BeanFactoryPostProcessor的集合
		List<BeanFactoryPostProcessor> nonOrderedPostProcessors = new ArrayList<>(nonOrderedPostProcessorNames.size());
		// 遍历普通的BeanFactoryPostProcessor名字的集合
		for (String postProcessorName : nonOrderedPostProcessorNames) {
			nonOrderedPostProcessors.add(beanFactory.getBean(postProcessorName, BeanFactoryPostProcessor.class));
		}
		// 遍历普通的BeanFactoryPostProcessor,执行ostProcessBeanFactory方法
